...
import cn.hualuomoli.mp.nideshop.base.mapper.UserBaseMapper;

// 
@Service(value = "cn.hualuomoli.mp.nideshop.base.service.UserBaseService")
...
public class UserBaseService {
  ...
  public <T extends User> int batchInsert(List<T> list, int fetchSize) {
    ...
    for (int i = 0; i < splits.size(); i++) {
      count += userBaseMapper.batchInsert(splits.get(i));
    }
    return count;
  }
  ...
  /** 根据主键id修改 */
  @Transactional(readOnly = false)
  public int updateById(java.lang.Integer id, User user) {
    Validate.notNull(id, "id is null.");
    Validate.notNull(user, "user is null.");
